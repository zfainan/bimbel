...
@section('content')
    <div class="card mb-4">
        <div class="card-header">
            <h3 class="h4">Tambah Jadwal Ajar</h3>
        </div>
        <div class="card-body" id="form">
            <form action="{{ route('jadwal-ajar.store') }}" method="POST">
                    ...
                    <div class="col-sm-10">
                        <select name="id_tentor" class="form-control @error('id_tentor') is-invalid @enderror" required>
                            @foreach ($tentor as $item)
                                <option @selected(old('id_tentor') == $item->id) value="{{ $item->id }}">{{ $item->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-sm-10">
                        <select name="hari" class="form-control @error('hari') is-invalid @enderror" required>
                            @foreach ($days as $item)
                                <option @selected(old('hari') == $item) value="{{ $item }}">{{ $item }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="row mb-3">
                    ...
                    <div class="col-sm-10">
                        <input type="time" value="{{ old('jam') }}" name="jam"
